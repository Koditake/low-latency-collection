        ...
        auto TCPSocket::sendAndRecv() noexcept -> bool {
            ...
            if (n_rcv > 0) {
                ...
                if (cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == SCM_TIMESTAMP && CMSG_LEN(sizeof(time_kernel))) {
                    ...
                    kernel_time = time_kernel.tv_sec * NANOS_TO_SECS + time_kernel.tv_usev * NANOS_TO_MICROS;
                }

                const auto user_time = getCurrentNanos();
                logger_.log("%:% %() % read socket:% len:% utime:% ktime:% diff:%\n", __FILE__, __LINE__, __FUNCTION__, common::getCurrentTimeString(&time_str_), fd_, next_rcv_valid_index_, user_time, kernel_time, (user_time - kernel_time));
            ...
            while (n_send > 0) {
                ...
                auto n = TCPSocket::send(fd_, send_buffer_, n_send_this_msg, flags);

                if (UNLIKELY(n < 0)) {
                    if (!wouldBlock())
                        send_disconnedted_ = true;
                    break;
                }
